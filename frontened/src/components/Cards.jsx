...
export const TemplateCard = ({ ThumbnailImg, isSelected, onSelect }) => {
  return (
    <div className={`group h-auto md:h-[300px] lg:h-[350px] flex flex-col bg-white border-2 overflow-hidden cursor-pointer transition-all duration-300 ease-in-out ${isSelected ? 'border-violet-500 shadow-lg' : 'border-gray-200 hover:border-violet-300'}`} onClick={onSelect}>
      {
        ThumbnailImg ? (<div className="relative w-full h-full overflow-hidden">
          <img src={ThumbnailImg || '/placeholder.svg'} alt="Template Review" className="w-full h-full object-cover group-hover:scale-110 transition-transform duaration-75" />
          <div className="absolute inset-0 bg-gradient-to-t from-white/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {
              isSelected && (
                <div className="absolute inset-0 bg-violet-100 flex items-center justify-center">
        ...
        </div>) : (<div className="w-full h-[200px] flex items-center justify-center bg-gradient-to-br from-violet-50 via-violet-100 to-fuchsia-100 rounded-lg shadow-md">
          <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-2xl flex items-center justify-center shadow-lg mb-3">
            <Edit className="text-white" size={18}></Edit>
          </div>
          <span className="text-gray-50 font-semibold"> No Preview</span>
